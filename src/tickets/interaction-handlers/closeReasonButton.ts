...
import {askAndWait} from "../../utils/utils";

export class CloseReasonButtonHandler extends InteractionHandler {
    ...
    public override async parse(interaction: ButtonInteraction) {
        if (!interaction.customId.startsWith("reason-close")) {
            return this.none();
        }
    ...
    public async run(interaction: ButtonInteraction): Promise<void> {
        ...
        const ownerUserId = interaction.customId.split("-")[3];
        if (!ownerUserId) {
        ...
        const reason = await askAndWait(interaction, "Please provide a reason for closing this ticket (you have 2 minutes):");
